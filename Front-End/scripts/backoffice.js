...
const id = allTheParameters.get('bookID')
...
if (id) {
  ...
  fetch(eventsURL + '/' + id)
    ...
    .then((eventDetails) => {
      ...
      document.getElementById('nomeEdAutore').value = eventDetails.nomeEdAutore // il nome dell'evento
      document.getElementById('genereCasaEditrice').value = eventDetails.genereCasaEditrice // la desc. dell'evento
      document.getElementById('price').value = eventDetails.price // il prezzo dell'evento
      document.getElementById('img').value = eventDetails.img // la data dell'evento
    })
    .catch((err) => {
...
class Concert {
  constructor(_nomeEdAutore, _genereCasaEditrice, _price, _img) {
    this.nomeEdAutore = _nomeEdAutore
    this.genereCasaEditrice = _genereCasaEditrice
    this.price = _price
    this.img = _img
  }
}
...
form.addEventListener('submit', (e) => {
  ...
  const nameInput = document.getElementById('nomeEdAutore') // input del nome
  const descriptionInput = document.getElementById('genereCasaEditrice') //  input descrizione
  const priceInput = document.getElementById('price') //  input prezzo
  const timeInput = document.getElementById('img') //  input data/ora
...
// funzione per mostrare anteprima da URL
...
// funzione per mostrare anteprima da file locale
